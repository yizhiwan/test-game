 ...
 * talks to /api/npc/* on its own origin. `output: "standalone"` traces only
 * the files a production server actually needs into .next/standalone, which
 * is what the Dockerfile copies into the Cloud Run image — without it the
 * image would need the full node_modules tree instead of a pruned one.
 *
